			...
			>
				...
				<template v-for="component in components" :key="component.id">
					<comp
						...
						@click="setSelected(component.id)"
						:id="component.id"
						:uid="component.id"
						:name="component.name"
					/>
	...
	methods: {
		download() {
			...
			for (let i in this.components) {
				files.push(generateFile(this.components[i]));
			}
			console.log(files);
			files.forEach((file, i) => {
				...
				a.setAttribute("download", `${this.components[i].name}.vue`);
				a.click();
		...
		setSelected(id) {
			console.log(id);
			let selectedObj = JSON.parse(
				JSON.stringify(
					this.components.reduce((acc, cur) => {
						if (acc) return acc;
						else if (cur.id === id) return cur;
					}, null)
				)
			);
